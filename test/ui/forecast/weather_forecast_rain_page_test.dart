  ...
  testWidgets("Rain page should contains widgets", (WidgetTester tester) async {
    await tester.pumpWidget(
      TestHelper.wrapWidgetWithLocalizationApp(
        WeatherForecastRainPage(setupWeatherForecastHolder(), 300, 100, false),
      ),
    );
    ...
    final RichText subtitle = tester
        .widget(find.byKey(const Key("weather_forecast_rain_page_subtitle")));
    ...
    expect(textSpan.children![1].toPlainText().contains("mm/h"), true);
    expect(textSpan.children![2].toPlainText().contains("max"), true);
    expect(textSpan.children![3].toPlainText().contains("mm/h"), true);
  });
}
